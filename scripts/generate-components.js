...
// This maps component names to their variants with image URLs
const componentData = {
  'Soup': {
    metadata: { name: 'Soup' },
    variants: {
...
function extractSVGContent(svgString) {
  // Extract inner content, removing outer <svg> tags and attributes
...
function generateComponent(componentName, componentInfo) {
  const fills = ['Outline', 'Solid', 'Tonal'];
  const styles = ['Sharp', 'Soft', 'Round'];
  
  const variantMap = {};
  for (const [key, url] of Object.entries(componentInfo.variants)) {
    variantMap[key] = url;
  }
...
${Object.entries(variantMap).map(([key, url]) => {
    // For now, we'll use a placeholder that will be replaced after fetching
    return `  '${key}': '', // Will be populated from ${url}`;
  }).join('\n')}
};
...
export function ${componentName}({ 
  ...
  return (
    <svg
      ...
      data-fill={fill}
      data-style={style}
      data-component-config={dataComponentConfig || '${componentInfo.metadata.name}'}
      {...props}
...
async function main() {
  const componentsDir = path.join(__dirname, '../src/icons');
  ...
  // First, fetch all SVGs and store them
  const svgData = {};
  
  for (const [componentName, componentInfo] of Object.entries(componentData)) {
    console.log(`Processing ${componentName}...`);
    svgData[componentName] = {};
    
    for (const [variantKey, url] of Object.entries(componentInfo.variants)) {
      try {
        const svgString = await fetchSVG(url);
        const svgContent = extractSVGContent(svgString);
        svgData[componentName][variantKey] = svgContent;
        console.log(`  ✓ Fetched ${variantKey}`);
      } catch (error) {
        console.error(`  ✗ Failed to fetch ${variantKey}:`, error.message);
        svgData[componentName][variantKey] = '';
      }
    }
  }
  ...
  for (const [componentName, componentInfo] of Object.entries(componentData)) {
    const componentCode = generateComponent(componentName, componentInfo);
    
    // Replace placeholder SVG content with actual content
    let finalCode = componentCode;
    for (const [variantKey, content] of Object.entries(svgData[componentName])) {
      const placeholder = `  '${variantKey}': '', // Will be populated from`;
      const replacement = `  '${variantKey}': ${JSON.stringify(content)},`;
